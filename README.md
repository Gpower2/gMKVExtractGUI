...
* **Localization & Live Language Switching:** UI labels, tooltips, popups, and context menus are loaded from JSON translation files and can be switched at runtime from the Options dialog. If locale files are missing, the app falls back to its embedded English defaults instead of showing localization placeholders.
...
4.  **Optional Setup:** Open **Options** to choose the UI theme and any available application language. The current build ships with `en`, `es`, `de`, `pt`, `pt-br`, `fr`, `el`, `cn`, and `ja` locale files.
